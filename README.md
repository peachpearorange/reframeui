...
* [`src/reframeui/styles.cljs`](src/reframeui/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/reframeui/`](src/reframeui/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/reframeui/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `reframeui` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/reframeui/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns reframeui.example
  (:require [reframeui.config :as config])
